...
if __name__ == "__main__":
    main()
"""
Объяснение команд при выбор действии из меню:

1.  Täisanalüüs - Выполняет полный анализ файлов проекта с указанным расширением. Выводится отчёт об анализе файлов (при запуске показывает: количество файлов, строки кода)  

2.  Salvesta raport - Сохраняет результат последнего анализа в файл-отчёт. ( Сообщение об успешном сохранении отчёта при запуске программы)

3.  Puhasta logid -  Удаляет или очищает все лог-файлы программы.(при запуске программы сообщает о том, что логи очищенны)

4.  Otsi faili algustähe järgi -  Ищет файлы, имена которых начинаются с указанной буквы.(при запуске показывает найденных файлов или сообщает, что таких файлов не было найдено)

0.  Välju -  Завершает работу программы
"""
